...
for item in test_list:
    cmd0 = 'mkdir {}'.format(item)
    os.system(cmd0)
    cmd1 = 'mkdir /scratch/ekaterina/{}'.format(location)
    os.system(cmd1)
    cmd2 = 'mkdir /scratch/ekaterina/{}/{}'.format(location, item)
    os.system(cmd2)
    cmd3 = 'cp /tudelft.net/staff-umbrella/abeellab/ekaterina/Experiment/Reference_data/{}/* /tudelft.net/staff-umbrella/abeellab/ekaterina/Experiment/{}/{}'.format(item, location, item)
    os.system(cmd3)
    genome = retrieve_seq (item)
    save_fasta (genome, "{}/{}.fasta".format(item, item))
